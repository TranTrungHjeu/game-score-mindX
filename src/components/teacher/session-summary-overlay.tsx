        ...
        >
          <div className="flex min-h-full items-center justify-center">
            <motion.section
              ...
              className="arena-panel relative w-full max-w-6xl overflow-hidden rounded-none border-2 border-white/80 p-6 text-slate-900 shadow-[0_32px_80px_rgba(15,23,42,0.34)] sm:p-8"
            >
              <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_15%_18%,rgba(170,118,16,0.2),transparent_18%),radial-gradient(circle_at_84%_12%,rgba(201,31,46,0.12),transparent_16%),radial-gradient(circle_at_50%_100%,rgba(170,118,16,0.1),transparent_20%)]" />

              <div className="relative grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(22rem,0.85fr)]">
                <div>
                  <div className="kid-chip text-sm font-bold text-red-800">Tổng kết buổi học</div>
                  ...
                  <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg">
                    {winner.name} khép lại buổi học với {formatPoints(winner.score)} và {resolveTeamVisual(winner).stage.name}
                    . Bạn có thể mở lại bảng chấm điểm hoặc bắt đầu một phiên mới ngay tại đây.
                  </p>

                  <div className="mt-6 grid gap-3 sm:grid-cols-3">
                    <div className="rounded-none border-2 border-white/90 bg-white/88 px-4 py-4">
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    ...
                    <button
                      type="button"
                      onClick={onReset}
                      className="rounded-none border-2 border-rose-200 bg-rose-50 px-6 py-3 font-bold text-rose-600"
                    >
                      Bắt đầu phiên mới
                    </button>
                  </div>
                ...
                <aside className="rounded-none border-2 border-white/80 bg-white/72 p-5">
                  ...
                  <div className="mt-4 space-y-3">
                    {rankedTeams.map((team, index) => {
                      ...
                      return (
                        <div
                          key={team.id}
                          className="rounded-none border-2 px-4 py-4"
                          style={{
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className="grid h-11 w-11 place-items-center rounded-none bg-slate-900 text-sm font-black text-white">
                              {style.rank}
                            </div>
                            <MascotAvatar
                              family={team.mascot}
                              level={visual.isMega ? stageCount : team.unlockedLevel}
                              stage={visual.stage}
                              size={96}
                              glow={index === 0}
                              animate={false}
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="mt-2 text-sm text-slate-600">
                                {visual.stage.name}
                                {visual.isMega ? " · Mega" : ` · Dạng ${team.unlockedLevel}/${stageCount}`}
                              </div>
                              <div className="mt-3 flex flex-wrap gap-2">
                                <span className="rounded-none bg-slate-900 px-3 py-1 text-xs font-black text-white">
                                  {formatPoints(team.score)}
                                </span>
                                <span className="rounded-none bg-white px-3 py-1 text-xs font-bold text-slate-600">
                                  {style.label}
                                </span>
                              </div>
                            </div>
